...
from src.data.scrolls_loader import ScrollsDataLoader
...
data_loader = ScrollsDataLoader(data_split=data_set)
number_of_books = data_loader.get_number_of_examples()
...
our_approach_output_dir = os.path.join('output' , 'scrolls' , "our_approach" , f'summarization_results_{data_set}')
llm_approach_output_dir = os.path.join('output' , 'scrolls', "llm_approach" , f'summarization_results_{data_set}')
wo_markov_approach_output_dir = os.path.join('output' , 'scrolls' , "wo_markov_approach" , f'summarization_results_{data_set}')
...
for index_to_fetch in range(number_of_books):
    ...
    try :
        book_summary_couple = data_loader.get_document_summary_pair(index_to_fetch)
        doc = book_summary_couple["document"]
        ...
        json_filename = os.path.join(wo_markov_approach_output_dir, f'doc_summary_{index_to_fetch}.json')
        with open(json_filename, 'w') as json_file:
            json.dump(result, json_file, indent=4)

        wo_markov_chain_approach_results.append(result)
    except Exception as e : 
        logger.error(f"Error processing book index {index_to_fetch}: {e}")
        continue
